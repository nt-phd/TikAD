...
const EXPORT_EQUIVALENT_ZOOM = TIKZ_PT_PER_UNIT / GRID_SIZE;
const ZOOM_LEVELS = [1, EXPORT_EQUIVALENT_ZOOM, 2, 3, 4, 5];
...
export type RenderPurpose = 'preview' | 'download-svg' | 'download-svg-plus';
...
export class LatexCanvas {
  ...
  private renderedSvgMarkup: string | null = null;
  private renderedSvgSource: string | null = null;
  ...
  getRenderedSvg(): string | null {
    return this.renderedSvgMarkup;
  }

  async renderSvgForExport(purpose: Extract<RenderPurpose, 'download-svg' | 'download-svg-plus'>): Promise<string | null> {
    const latex = this.latexDoc.toFullSource();
    try {
      const anchorRequests = this.buildAnchorRenderRequests();
      const data = await this.requestRender(latex, anchorRequests, purpose);
      if (data.svg) {
        this.renderedSvgMarkup = data.svg;
        this.renderedSvgSource = latex;
        ...
        if (data.anchorError) console.warn('[LatexCanvas] export probe warnings:', data.anchorError);
        this.showError(null);
        return data.svg;
      }
      console.warn('[LatexCanvas] export render error:', data.error);
      this.showError(data.error ?? 'LaTeX error');
      return null;
    } catch (e) {
      console.warn('[LatexCanvas] export render server unreachable:', e);
  ...
  private async doRender(): Promise<void> {
    ...
    try {
      const anchorRequests = this.buildAnchorRenderRequests();
      const data = await this.requestRender(latex, anchorRequests, 'preview');
      if (data.svg) {
        this.renderedSvgMarkup = data.svg;
        this.renderedSvgSource = latex;
        this.injectSvg(data.svg, data.tx ?? 0, data.ty ?? 0);
  ...
  private async requestRender(latex: string, anchors: AnchorRenderRequest[], purpose: RenderPurpose): Promise<RenderResponse> {
    const forceRender = purpose !== 'preview';
    const res = await fetch(`${RENDER_SERVER_URL}/render`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ latex, anchors, purpose, forceRender }),
  ...
  private injectSvg(svgText: string, tx: number, ty: number): void {
    ...
    const renderSizePt = this.getSvgRenderedSizePt(svgEl);
    if (renderSizePt) {
      const widthPx = renderSizePt.width * ptToPx;
      const heightPx = renderSizePt.height * ptToPx;
  ...
  private setGhostLatexPreview(preview: GhostLatexPreview | null): void {
    ...
    const renderSizePt = this.getSvgRenderedSizePt(svgEl);
    if (renderSizePt) {
      svgEl.style.width = `${renderSizePt.width * ptToPx}px`;
      svgEl.style.height = `${renderSizePt.height * ptToPx}px`;
    }
    svgEl.removeAttribute('width');
    svgEl.removeAttribute('height');
  ...
  private getSvgRenderedSizePt(svgEl: SVGSVGElement): { width: number; height: number } | null {
