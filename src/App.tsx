...
const TIKAD_SVG_ENCODING = 'base64';

function encodeUtf8ToBase64(source: string): string {
  const bytes = new TextEncoder().encode(source);
  let binary = '';
...
function decodeUtf8FromBase64(source: string): string {
...
function buildSvgPlusMarkup(svgMarkup: string, latexSource: string): string {
  try {
    ...
    metadata.textContent = JSON.stringify({
      app: 'TikAD',
      encoding: TIKAD_SVG_ENCODING,
      format: TIKAD_SVG_FORMAT,
      source: encodeUtf8ToBase64(latexSource),
...
function extractLatexSourceFromSvgPlus(svgMarkup: string): string | null {
  try {
    ...
    for (const metadata of metadataCandidates) {
      ...
      if (!raw) continue;
      try {
        const parsed = JSON.parse(raw) as { encoding?: string; format?: string; source?: string; version?: number };
        if (parsed.format !== TIKAD_SVG_FORMAT) continue;
        if (parsed.version !== TIKAD_SVG_VERSION) continue;
        if (parsed.encoding !== TIKAD_SVG_ENCODING) continue;
        if (typeof parsed.source === 'string' && parsed.source.trim()) return decodeUtf8FromBase64(parsed.source);
      } catch {
        continue;
      }
    }
  ...
  return (
    <>
      <ToolbarView
        ...
        onOpenResetDialog={appState.onOpenResetDialog}
